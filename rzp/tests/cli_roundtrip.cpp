...
int main() {
    ...
    assert(std::system(cmd_decode.c_str()) == 0);

    std::ifstream orig(in_png, std::ios::binary);
    std::vector<char> v1((std::istreambuf_iterator<char>(orig)), {});
    std::ifstream round(out_png, std::ios::binary);
    std::vector<char> v2((std::istreambuf_iterator<char>(round)), {});
    assert(v1 == v2);

    std::cout << "rzp CLI demo roundtrip passed\n";
    return 0;
} 